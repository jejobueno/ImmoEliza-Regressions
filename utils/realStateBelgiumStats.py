...
def splitByRegions(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    return {
        "Wallonia": housing_wallonia,
        "Brussels": housing_brussels,
        "Flanders": housing_flanders,
    }


def createDataFrameStat(
    original_df: pd.DataFrame, df_splited: pd.DataFrame, region: str
):
